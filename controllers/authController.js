...
exports.restrictTo =
  (...roles) =>
  (req, res, next) => {
    // check if `currentUser` has not permission
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    // MEMENTO : `user` is defined with the find of `req.body.email` above
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 3) Update `changedPasswordAt` property for the user // this actually happens with the `pre-save` middleware in the `userModel.js`
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!(await user.correctPassword(req.body.passwordCurrent, user.password))) {
    return next(
      new AppError(
        '`CurrentPassword` field not matching with user password. Please retry!',
        401,
      ),
    );
  }
